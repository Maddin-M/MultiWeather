...
public class CommandUnlock implements Command {
    ...
    public boolean run(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        affectedWorlds.forEach(world -> {
            if (!isWeatherLockedInWorld(world)) {
                sender.sendMessage(getMessage("weather_unlock_already_unlocked", world.getName()));
            } else {
                world.setGameRule(GameRule.DO_WEATHER_CYCLE, true);
                sender.sendMessage(getMessage("weather_unlock_success", world.getName()));
            }
        });
        return true;
    }
    ...
    public String getHelp() {
        return getMessage("weather_unlock_help", COMMAND);
    }
}
